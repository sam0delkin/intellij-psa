...
of the currently focused [PSI element](https://plugins.jetbrains.com/docs/intellij/psi-elements.html), called PSA Context, then JSON encode it,
...
sequenceDiagram
  ...
  participant IDE
  participant Plugin
  participant PSA Script
  User->>IDE: What are completions for this element?
  Note right of User: User is pressed <br/> ctrl/cmd + space on the element
  IDE->>Plugin: Get completions
  Note right of IDE: User is asking for completions <br/> for currently focused PSI element
  Plugin->>PSA Script: Get completions
  Note right of Plugin: Generating PSA Context <br/> from currently focused PSI element,<br/> sending it to the PSA script
  Note right of PSA Script: Analyzes PSA context,<br/> returns completions
  PSA Script->>Plugin: Here are completions
  Note right of Plugin: Simple JSON content <br/> of completions is returned
  Plugin->>IDE: Return completions
  Note right of IDE: Plugin converts JSON into <br/> meaningful completions
  IDE->>User: Return completions
  Note right of User: User-friendly list of <br/> completions is shown
```
...
See [Swagger](https://swagger.io/) documentation [here](https://petstore.swagger.io/?url=https://raw.githubusercontent.com/sam0delkin/intellij-psa/refs/heads/main/doc/schema.yaml).
...
To help with building static completion patterns, a new action were added to the File Editor menu, called `Generate Pattern Model`:
...
Also, you may use `any_parent`, `any_prev` and `any_next` fields to match to any parent, previous or next element.
...
is same, then GoTo action is performed. This is not a good approach in some cased, so that's why a new option were
introduced, called `matcher`.

Internally, Intellij IDEA is using [Apache Velocity](https://velocity.apache.org/) templating engine to some internal
file/code templates. So, plugin is using Apache Velocity as well, and provides both completion and element data as
template variables. So, for example, you can match element text by regular expression using `matcher` field:

```JSON
{
  "matcher": "$completion.text.matches($model.text)"
}
```
